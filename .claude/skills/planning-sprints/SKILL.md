...
```bash
.claude/scripts/gpt-review-api.sh sprint /tmp/sprint-draft.md \
  --augmentation /tmp/gpt-sprint-augmentation.md \
  --iteration 1 > /tmp/gpt-response-1.json
```

**Parse Response:**
- `verdict`: APPROVED | CHANGES_REQUIRED
- `blockers`: Array of dependency issues/missing tasks (not style issues)
...
**If CHANGES_REQUIRED:**
1. Read the blockers from the response
2. Fix ONLY the blockers identified (not style, not formatting)
3. Re-save draft to `/tmp/sprint-draft.md`
...
6. GPT now receives:
   - The updated sprint plan draft
   - Its previous findings
   - Instruction to ONLY check if previous blockers were fixed
